...
(function () {
  ...
  1: [function (require, module, exports) {
    window.onload = function () {
      ...
      var email = document.querySelector('#email'); // const errorMessageEmail = document.querySelector('.wrong-email');
      ...
      function handleFormSubmit(form) {
        ...
        var values = validate.collectFormValues(form); // затем мы проверяем их на соответствие ограничениям

        var errors = validate(values, constraints); // Затем мы обновляем форму, чтобы отразить результаты

        showErrors(form, errors || {}); // И если все ограничения пройдут, мы сообщим пользователю
      ...
      function showErrorsForInput(formData, errors) {
        ...
        console.log(closestParent(formData.parentNode)); // Найдите, куда сообщения об ошибках будут вставлены в
        // var messages = formGroup.querySelector(".messages");
        // Сначала мы удаляем все старые сообщения и сбрасываем классы

        resetFormGroup(formGroup); // Если у нас есть ошибки

        if (errors) {
          // мы сначала отмечаем, что в форме есть ошибки
          formGroup.classList.add("has-error"); // затем мы добавляем все ошибки
      ...
      function resetFormGroup(formGroup) {
        ...
        formGroup.classList.remove("has-success"); // и удалите все старые сообщения
